    ...
    return (
        <section className={`${theme.layout.heroSection} ${className}`}>
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-left z-10 w-full max-w-xl px-6">
                {config.hero.title.map((line, index) => (
                    <span key={index} className={`block ${theme.typography.brand.hero} ${spectral.className}`}>
                        {line}
                    </span>
                ))}
                {config.hero.subtitle.map((line, index) => (
                    <span key={index} className={`block ${theme.typography.brand.heroSecondary} ml-[15%] ${index === 0 ? 'mt-[2%]' : 'mt-[8px]'}`}>
                        {line}
                    </span>
                ))}
            </div>
